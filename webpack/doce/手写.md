...
- 首先会把模板转换为 AST 抽象语法树，
- 然后把 AST 转换为可执行的 render 函数，
- 最后才可以生成真是 DOM

通过上面我们知道，webpack在打包vue文件，会直接将 template转成了 render 函数，这样再浏览器中就可以直接生成对应的html了

那我们就先来说说 render函数是如何输出成html的

再次之前我们先看看， `template` 、 ast语法树， 分别是什么
```vue
<!--// template 就是 vue 的模板语法-->
<template>
 <div class="box">
...
const f = {
 tag: 'div', // html标签
 attrs: [{name: 'class', value: 'box'}], // html中的属性和对应的值
 type: 1, // // 标识元素1 和文本type 3
 children: [  // 子节点，  
  {
   tag: 'h3',
