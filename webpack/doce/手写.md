...
> 在体检webpack之前，我们大致了解以下webpack 4个核心概念
...
了解了webpack的基本使用和他的运行原理，那在webpack结合vue开发时，配置了vue-loader为什么还需要 VueLoaderPlugin呢？

### 通过 vue-loader 解析 .vue文件 为什么还需要 VueLoaderPlugin

1. 通过上面 loader 和plugins 的了解， 我们知道， loader只是将除了js以外文件转换成 webpack调用的模块
2. loader肯定是将处理.vue模块， 那VueLoaderPlugin 呢

通过查看源码分析， webpack启动时，会构建和组册插件，VueLoaderPlugin 中
![img.png](./model/img.png)


## vue在运行时，是如何解析`template` 成 html的
